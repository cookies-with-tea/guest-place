...
async function main() {
  try {
    ...
    const writeFile = (file, content) => {
      fs.writeFileSync(path.join(appDir, file), typeof content === 'string' ? content : JSON.stringify(content, null, 2) + '\n')
    }
    ...
    writeFile('env.d.ts', `/// <reference types="vite/client" />\n\ndeclare module '*.vue' {\n  import type { DefineComponent } from 'vue'\n  export default DefineComponent\n}\n`)
    writeFile('eslint.config.mjs', `import { createConfig } from '@admin-panel/eslint-config'\n\nexport default createConfig()\n`)
    writeFile('stylelint.config.mjs', `import { createConfig } from '@admin-panel/stylelint-config'\n\nexport default createConfig()\n`)

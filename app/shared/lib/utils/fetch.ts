...
export const fetchData = async <T>(url: string, options?: JsonFetchOptions): Promise<IResponse<T>> => {
  try {
    const body = options?.body ? camelToSnake(options.body) : undefined

    const response = await $fetch<IResponse<T>>(url, {
      ...options,
      body,
      responseType: 'json',
    })

    return snakeToCamel(response) as IResponse<T>
  } catch (error: any) {

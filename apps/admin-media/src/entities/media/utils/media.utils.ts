...
export const mediaUtils = {
  ...
  isImage: (file: File | string): boolean => {
    if (typeof file === 'string') {
      return file.endsWith('.jpg') || file.endsWith('.jpeg') || file.endsWith('.png') || file.endsWith('.gif') || file.endsWith('.webp') || file.endsWith('.svg')
    }
  ...
  validateMediaFile: (file: File): { valid: boolean; message?: string } => {
    ...
    const allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/svg+xml', 'video/mp4', 'video/webm', 'video/avi', 'video/mov']
